...
I recently discovered a new front end framework: [VueJS](http://vuejs.org/). So far it meets my expectations and then some! But as any tool it has some nuances of which you must be mindful. 

Here are some for version **2.0.2** in reverse chronological order of discovery (for version 1.x look below)

### Nuance 10 - Updating from 2.0.1 to 2.0.2

When I updated only Vue in my `package.json`, it started complaining about `vue-template-compiler` being outdated.
For this, I simply changed the vue-loader version in my `package.json` to `^9.5.3` and ran `npm install`.
- - -
Here are a few for version **1.0.16**.
